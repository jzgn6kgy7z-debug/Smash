...
const copy=(key,text)=>{navigator.clipboard.writeText(text).then(()=>{setCopied(s=>({...s,[key]:true}));setTimeout(()=>setCopied(s=>({...s,[key]:false})),2000);showToast('Copied!')})};
...
const handleCreate=async()=>{if(!newS.date||!newS.time||!newS.duration||!newS.spots||!newS.location_name)return;await supabase.from('sessions').insert({...newS,duration:parseFloat(newS.duration),spots:parseInt(newS.spots),signups:[],waitlist:[],matches:null});setNewS({date:'',time:'',duration:'',spots:'',location_name:'',location_address:''});setShowNew(false);showToast('Session created! 🎾')};
...
const btn=(c='#a8d8a8',bg='#2a5a2a',b='#4a8a4a')=>({background:bg,border:`1px solid ${b}`,color:c,fontFamily:'inherit',fontSize:12,letterSpacing:2,textTransform:'uppercase',padding:'9px 16px',cursor:'pointer',borderRadius:4});
if(!currentUser)return(<div style={{minHeight:'100vh',background:'#0a0a0a',fontFamily:"'Georgia',serif",color:'#f0ece0',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',padding:32}}><div style={{fontSize:48,marginBottom:16}}>🎾</div><h1 style={{fontSize:38,fontWeight:400,letterSpacing:3,color:'#e8f5e8',marginBottom:8}}>Smash</h1><p style={{color:'#5a7a5a',marginBottom:32,textAlign:'center'}}>Enter your name to join</p><div style={{width:'100%',maxWidth:320}}><input value={''} onChange={e=>e.target.value} onKeyDown={e=>e.key==='Enter'&&e.target.value.trim()&&handleEnter(e.target.value.trim())} placeholder="Your name..." style={{...inp,fontSize:18,marginBottom:12,textAlign:'center'}} autoFocus ref={el=>{if(el)el.oninput=ev=>el._val=ev.target.value}}/><button onClick={()=>{const el=document.querySelector('input');if(el&&el.value.trim())handleEnter(el.value.trim())}} style={{...btn('#a8d8a8','#2a5a2a','#4a8a4a'),width:'100%',fontSize:14,padding:12}}>Enter Smash →</button></div></div>);
...
<div style={{display:'flex'}}>{TABS.map(t=><button key={t} onClick={()=>setTab(t)} style={{background:'none',border:'none',borderBottom:tab===t?'2px solid #7ab87a':'2px solid transparent',color:tab===t?'#a8d8a8':'#5a7a5a',fontFamily:'inherit',fontSize:13,letterSpacing:2,textTransform:'uppercase',padding:'10px 16px 8px',cursor:'pointer'}}>{t}</button>)}</div>
</div></div>
...
<button onClick={()=>copy(`ann-${session.id}`,announceMsg)} style={btn(copied[`ann-${session.id}`]?'#25d366':'#5a9a5a','#0d200d','#25d36655')}>{copied[`ann-${session.id}`]?'✓ Copied!':'📋 Copy'}</button>
...
<div style={{display:'flex',gap:8,flexWrap:'wrap'}}>
<button onClick={()=>copy(`upd-${session.id}`,updateMsg)} style={btn(copied[`upd-${session.id}`]?'#25d366':'#5a9a5a','#0d200d','#25d36655')}>{copied[`upd-${session.id}`]?'✓ Copied!':'📋 Copy Update'}</button>
...
<button onClick={()=>handleGenMatches(matchSession)} style={btn('#7ab8d8','#1a2a3a','#2a4a6a')}>↺ Re-shuffle</button>
