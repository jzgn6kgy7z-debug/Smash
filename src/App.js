...
const LEVEL_COLORS = { 1:'#ffd700', 2:'#ff9f40', 3:'#7ab8d8', 4:'#6b9ef0', 5:'#a88ad8', 6:'#d87a7a' };
...
const CSS = `
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  @keyframes pulse { 0%,100%{transform:scale(1);box-shadow:0 0 0 0 rgba(91,141,238,0.3)} 50%{transform:scale(1.02);box-shadow:0 0 0 8px rgba(91,141,238,0)} }
  @keyframes shimmer { from{background-position:-200% 0} to{background-position:200% 0} }
  @keyframes slideIn { from{opacity:0;transform:translateX(-12px)} to{opacity:1;transform:translateX(0)} }
  @keyframes glow { 0%,100%{box-shadow:0 0 5px rgba(91,141,238,0.25)} 50%{box-shadow:0 0 20px rgba(91,141,238,0.4)} }
  ...
  .card:hover { transform: translateY(-3px); box-shadow: 0 12px 32px rgba(0,0,0,0.4), 0 0 0 1px rgba(91,141,238,0.12); }
  ...
  .tab-btn::after { content:''; position:absolute; bottom:0; left:50%; right:50%; height:2px; background:#6b9ef0; transition: all 0.2s ease; border-radius:2px 2px 0 0; }
  .tab-btn.active::after { left:0; right:0; }
  input, select { transition: border-color 0.2s, box-shadow 0.2s; }
  input:focus, select:focus { border-color: #6b9ef0 !important; box-shadow: 0 0 0 3px rgba(91,141,238,0.12) !important; outline: none; }
  ...
  ::-webkit-scrollbar-thumb { background: #1e3078; border-radius: 3px; }
  .quote-anim { animation: quoteSlide 5s ease-in-out infinite; }
  .glow-green { animation: glow 2s ease infinite; }
  .shimmer { background: linear-gradient(90deg, transparent 0%, rgba(91,141,238,0.06) 50%, transparent 100%); background-size: 200% 100%; animation: shimmer 2s infinite; }
...
function QuoteBanner() {
  ...
  const q = QUOTES[qi];
  return (
    <div style={{ background:'linear-gradient(135deg,#0e1e38,#0a1f0a)', border:'1px solid #1e4a1e', borderRadius:12, padding:'14px 18px', marginBottom:20, display:'flex', alignItems:'center', gap:14, overflow:'hidden', position:'relative' }}>
  ...
  return (
    <div>
      <div style={{ display:'flex', gap:8, justifyContent:'center', marginBottom:10 }}>
        {LEVELS.map(l => (
          <button key={l} className={`level-btn${value===l?' active':''}`} onClick={() => onChange(l)} style={{ width:size, height:size, borderRadius:'50%', border:`2px solid ${value===l?LEVEL_COLORS[l]:'#2a3a2a'}`, background:value===l?LEVEL_COLORS[l]+'33':'#0d1c32', color:value===l?LEVEL_COLORS[l]:'#243c8a', fontFamily:'Inter,sans-serif', fontSize:size>40?17:14, fontWeight:800, cursor:'pointer' }}>{l}</button>
        ))}
      </div>
      <div style={{ textAlign:'center', fontSize:12, color:LEVEL_COLORS[value]||'#2c4c98', fontWeight:600, fontFamily:'Inter,sans-serif', minHeight:20 }}>{LEVEL_LABELS[value]||''}</div>
    </div>
  );
}
...
export default function App() {
  ...
  const showToast = (msg, color='#6b9ef0', emoji='🎾') => { setToast({msg,color,emoji}); setTimeout(()=>setToast(null),2500); };
  ...
  const handleSignUp = async(session)=>{
    ...
    else{ us=[...signups,currentUser]; showToast("You're in! Let's smash!",'#6b9ef0','🔥'); }
  ...
  const handleCreate = async()=>{
    ...
    showToast("Session created! Who's ready?",'#6b9ef0','🏟️');
  };
  ...
  const handleAddPlayer = async()=>{
    ...
    setNewPlayer({name:'',phone:'',level:3}); showToast('Player added!','#6b9ef0','👋');
  };
  const handleRemovePlayer = async(id)=>{ await supabase.from('players').delete().eq('id',id); loadPlayers(); };
  const handleUpdateLevel = async(player,newLevel)=>{ await supabase.from('players').update({level:newLevel}).eq('id',player.id); loadPlayers(); showToast(`Level updated to ${LEVEL_LABELS[newLevel]}`,'#6b9ef0','⭐'); };
      ...
      <div style={{ background:'rgba(10,15,25,0.98)', borderBottom:'1px solid #142458', backdropFilter:'blur(10px)', position:'sticky', top:0, zIndex:100 }}>
        <div style={{ maxWidth:600, margin:'0 auto', padding:'0 16px' }}>
          ...
          <div style={{ display:'flex', gap:4 }}>
            {TABS.map(t=>(
              <button key={t} className={`tab-btn${tab===t?' active':''}`} onClick={()=>setTab(t)} style={{ background:'none', border:'none', color:tab===t?'#b8d4f8':'#223c82', fontFamily:'Inter,sans-serif', fontSize:12, fontWeight:tab===t?700:500, letterSpacing:1.5, textTransform:'uppercase', padding:'10px 16px 12px', cursor:'pointer' }}>{t}</button>
            ))}
          </div>
        </div>
      </div>
        ...
        {!loading && tab==='Sessions' && (
          <div className="fade-up">
            ...
            <div style={{ display:'flex', gap:10, marginBottom:20 }}>
              <StatPill label="Sessions" value={sessions.length} color="#6b9ef0" />
                ...
                <div key={session.id} className="card fade-up" style={{ background:'#101828', border:'1px solid #142458', borderRadius:16, marginBottom:16, overflow:'hidden', animationDelay:`${si*0.05}s` }}>
                  ...
                  <div style={{ padding:'16px 18px' }}>
                    ...
                    <div style={{ display:'flex', flexWrap:'wrap', gap:8, marginBottom:panelOpen?12:0 }}>
                      <button className="btn-hover" onClick={()=>handleSignUp(session)} style={{ background:isIn?'#2a1a1a':isWaiting?'#1a150a':isFull?'#121212':'linear-gradient(135deg,#0f2a5a,#0a1a48)', border:`1px solid ${isIn?'#5a2a2a':isWaiting?'#4a3a0a':isFull?'#2a2a2a':'#3a68c0'}`, color:isIn?'#ff7a7a':isWaiting?'#c8a86a':isFull?'#3a3a3a':'#b8d4f8', fontFamily:'Inter,sans-serif', fontSize:12, fontWeight:700, letterSpacing:1, textTransform:'uppercase', padding:'10px 16px', cursor:isFull&&!isIn&&!isWaiting?'default':'pointer', borderRadius:10 }}>
                      ...
                      {signups.length>=4 && (
                        <button className="btn-hover" onClick={()=>session.matches?setViewMatches(session.id):handleGenMatches(session)}
                          style={{ background:'linear-gradient(135deg,#0f2060,#0c1a48)', border:'1px solid #2a4a7a', color:'#7ab8d8', fontFamily:'Inter,sans-serif', fontSize:12, fontWeight:700, letterSpacing:1, textTransform:'uppercase', padding:'10px 16px', cursor:'pointer', borderRadius:10 }}>
                      ...
                      <button className="btn-hover" onClick={()=>setOpenPanel(panelOpen&&panelType==='share'?null:{id:session.id,type:'share'})}
                        style={{ background:'#0a180a', border:`1px solid ${panelOpen&&panelType==='share'?'#25d366':'#142458'}`, color:'#25d366', fontFamily:'Inter,sans-serif', fontSize:12, fontWeight:700, letterSpacing:1, textTransform:'uppercase', padding:'10px 14px', cursor:'pointer', borderRadius:10 }}>
                        🔗 Share
                      </button>
                      <button className="btn-hover" onClick={()=>setOpenPanel(panelOpen&&panelType==='wa'?null:{id:session.id,type:'wa'})}
                        style={{ background:'#0a180a', border:`1px solid ${panelOpen&&panelType==='wa'?'#25d366':'#142458'}`, color:'#25d366', fontFamily:'Inter,sans-serif', fontSize:12, fontWeight:700, letterSpacing:1, textTransform:'uppercase', padding:'10px 14px', cursor:'pointer', borderRadius:10 }}>
                        📲 WA
                      </button>
                    </div>
                    ...
                    {panelOpen && panelType==='wa' && (
                      <div className="slide-in" style={{ background:'#111e2e', border:'1px solid #25d36622', borderRadius:12, overflow:'hidden' }}>
                        ...
                        {[
                          ...
                          <div key={action.label} style={{ display:'flex',alignItems:'center',justifyContent:'space-between',padding:'12px 16px',borderBottom:'1px solid #0c1628',opacity:action.disabled?0.3:1 }}>
        ...
        {!loading && tab==='Players' && (
          <div className="fade-up">
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
              {players.map((p,i)=>(
                <div key={p.id||p.name} className="card" style={{ background:p.name===currentUser?'#0a1f0a':'#0c1828', border:`1px solid ${p.name===currentUser?'#2e56b0':'#121e48'}`, borderRadius:12, padding:'14px 16px' }}>
